...
```{r}
hist_with_box('age', 30, 5, 0.3, 1, c(0,35), 'Age (Years)')
```
...
plot_frq(q7.to.plot$q7, sort.frq="desc",  ylim = c(0,110), show.na = TRUE, wrap.labels = 10) + xlab('Gender') + ylab("Frequency")
rm(q7.to.plot)
```
</div>
...
plot_frq(tmp$hispanic,  ylim = c(0,110)) + xlab("") + ylab("Frequency")
```
</div>
...
plot_frq(bns$q11, ylim=c(0,150)) + xlab('Number of Dependents') + ylab("Frequency")
```
...
plot_frq(dep.ages.df$Ages, type='histogram', show.mean = TRUE, ylim=c(0,8), show.mean.val = FALSE, show.sd = FALSE) +
  geom_boxplot(data=dep.ages.df, aes(x=Ages, y=6.5), alpha=0.6, width=1.5, fill="#336699") +
  geom_jitter(data=dep.ages.df, aes(x=Ages, y=6.5), position=position_jitter(height=.8)) +
