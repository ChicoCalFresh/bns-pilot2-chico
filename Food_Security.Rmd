...
plot_frq(bns$two.item, ylim=c(0,100)) + xlab('Food Insecure') + ylab("Frequency")
```
</div>
...
```{r, fig.width=8}
plt1 <- plot_frq(bns$q67, ylim=c(0,80), 
                 title="Did you cut/skip any meals") + xlab('') + ylab("Frequency")

plt2 <- hist_with_box('q68', 15, 2.5, 0.4, NULL, c(0,18), 'Number of days meals were cut/skipped')
...
```{r, fig.width=8}
plt1 <- plot_frq(bns$q72, ylim=c(0,125), title="Did you not eat for a whole day", wrap.title = 20) + xlab('') + ylab("Frequency")

plt2 <- hist_with_box('q73', 5, 1, .4, NULL, c(0,6), 'Number of days all meals were cut/skipped')
