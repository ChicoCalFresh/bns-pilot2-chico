...
plot_frq(bns$q23, ylim = c(0,100)) + xlab('') + ylab("Frequency")
```
</div>
...
p1 <- plot_frq(bns$q34, ylim = c(0,125), wrap.title=40, wrap.labels =12, 
               title = 'How many times have you been unsure of where you were going to sleep at night?') + xlab('') + ylab("Frequency")
p2 <- plot_frq(bns$q35, wrap.labels =10, ylim = c(0,75), title = 'How many times have you moved?') + xlab('')
...
```{r, fig.width=8.3}
plot_frq(bns$q37, ylim = c(0,30), wrap.labels = 17, sort.frq = 'desc') + xlab('') + ylab("Frequency")
```
...
bns %>% 
  ...
  pivot_longer(col=c(homelessness_month, homelessness_year, housing_insecure)) %>%
  arrange(desc(value)) %>%
